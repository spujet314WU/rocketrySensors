...
static int8_t init_bmi08(struct bmi08_dev* bmi08dev)
{
    int8_t rslt;
...
static int8_t enable_bmi08_interrupt(struct bmi08_dev* bmi08dev, struct bmi08_accel_int_channel_cfg* accel_int_config)
{
    int8_t rslt;
...
void bmi_init(struct bmi08_dev* bmi08dev){
    ...
    if (rslt == BMI08_OK)
    {
        ...
        rslt = enable_bmi08_interrupt(bmi08dev,&accel_int_config);
...
void BMI_ACC_FIFO(struct bmi08_dev* bmi08dev, struct bmi08_fifo_frame* fifo_frame, uint8_t* status, uint8_t fifo_data[BMI08_ACC_FIFO_RAW_DATA_BUFFER_SIZE], uint16_t* wml, struct bmi08_sensor_data* bmi08_accel){
    uint8_t rslt;
    ...
    if (*status & BMI08_ACCEL_FIFO_WM_INT)
    {
        ...
        if (rslt == BMI08_OK)
        {
            ...
            for (uint16_t idx = 0; idx < accel_length; idx++)
            {
...
//         /*Enable data ready interrupts*/
//         rslt = enable_bmi08_interrupt(bmi08dev);
//         bmi08_error_codes_print_result("enable_bmi08_interrupt", rslt);
